 ...
 * Date: 20/11/12
 */
class Filters_File_Resize implements Zend_Filter_Interface
{
    ...
    protected $_adapter = 'Filters_File_Gd';
     ...
     * @return Filters_File_Resize An instance of this filter
     */
    public function __construct($options = array())
    {
        ...
        if (isset($options['adapter'])) {
            if ($options['adapter'] instanceof Filters_File_Adapter) {
                $this->_adapter = $options['adapter'];
            } else {
                $name = $options['adapter'];
                if (substr($name, 0, 33) != 'Filters_File_') {
                    $name = 'Filters_File_Adapter_' . ucfirst(strtolower($name));
                }
                $this->_adapter = $name;
            }
        }
    ...
    {
        if ($this->_adapter instanceof Filters_File_Adapter) {
            return;
        } else {
